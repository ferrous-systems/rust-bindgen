...
fn var_diagnostics(item: &str, line: usize, origin: &str, msg: &str, note: &str, emit_diagnostics: bool) {
    if emit_diagnostics {
        let mut slice = crate::diagnostics::Slice::default();
        slice.with_source(item).add_line_start(line).add_origin(origin);
        Diagnostic::default()
